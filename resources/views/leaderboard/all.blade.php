	...
	<script type="text/x-template" id="TableView">
		<div id="leaderboard-wrapper">
			<div id="leaderboard-wrapper-nav">
				<ul style="background: #ffd0ac !important;">
					<li id="leaderboard-top-score-button" 
						style="width: calc(100% - 20px)  !important;"
						class="">{{ $title }} - @{{ $store.getters.currentTab }}
						@if($path == 'tournament')
							<br>
								<a style="font-size: 14px; color: black; font-weight: 100;">
									Waktu Mulai: {{ $tournament->day_begin }} - {{ $tournament->time_begin }} s/d
									Waktu Selesai: {{ $tournament->day_end }} - {{ $tournament->time_end }} 
								</a>   
						@endif
					</li>
				</ul>
		...
		const store = new Vuex.Store({
			...
			actions: {
				...
				onRequestData(context) {
					axios.get('{{ url("/api/leaderboard/all") }}')
